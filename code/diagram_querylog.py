...
class DiagramQueryLog(Diagram):
    ...
    def initLogFile(self):
        filename = './log/on-base/' + self.db_name + '/diagram/qlog-' + str(self.query_id) + '-' + str(
    ...
    def pqoByFeatureCollection(self, N):
        ...
        filename = f'./reuse/{self.db_name}/diagram/qlog/qlog-{self.query_id}-{self.template_id}_{self.workload_name}_b{self.b}_N{self.N}.json'
    ...
    def collectPlans(self):
        # Don't add the optimal plan at each sample 
        # just add the opimal plan from each query!
        for sql_id, para_sql in enumerate(self.queries_train):
            ...
            self.plan_list.add(base_pg_plan)
    ...
    '''
    For each plan, collect the cost of this plan at each query
    '''
    ...
    def collectPlanCost(self):
        for plan in self.plan_list:
            cost_list = []
            for sql_id, para_sql in enumerate(self.queries_train):
                recost_at_query, _, _ = get_plan_cost_simple(self.cursor, sql=para_sql, hint=plan, explain=explain,
                                                             debug=True)
                cost_list.append(recost_at_query)
            self.costCollection.append(cost_list)
