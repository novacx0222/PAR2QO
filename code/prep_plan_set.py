...
def enum(sensitive_dims, table_name_id_dict, join_maps, join_info, est_base_card, raw_base_card, f_base_sel,
         est_join_card, raw_join_card, f_join_sel, to_execute_, db_name, err_samples, recentered_err, debug=False):
    ...
    with conn.cursor() as cursor:
        ...
        for i in range(len(err_samples)):
            ...
            try:
                ...
                join_order, _, scan_mtd = decode(query_plan[0][0][0]['Plan']['Plans'],
                                                 query_plan[0][0][0]['Plan']['Node Type'])
...
def get_plan_set_by_enum(table_name_id_dict, join_maps, join_info,
    ...
    cost_list, plan_change_sel_list, plan_list = enum(dims, table_name_id_dict, join_maps, join_info, est_base_card,
                                                      raw_base_card, f_base_sel,
                                                      est_join_card, raw_join_card, f_join_sel, to_execute_, db_name,
                                                      joint_error_sample, recentered_err)
