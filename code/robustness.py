...
parser.add_argument('--find_anchors', action='store_true')
parser.add_argument('--naive', action='store_true')
parser.add_argument('--rqo_query', type=str, required=False, default=None,
                    help='The path to the query that you want to calculate RQO')
parser.add_argument('--template_id', default=None, type=int)
...
if args.cal_sen == 'morris' or args.method == 'morris':
    cal_sen_method = '_morris'
    if db_name == 'imdbloadbase':
        sen_dict_name = "sen_dict_morris"
    if db_name == 'dsb':
        sen_dict_name = "dsb_sen_dict_morris"
    if db_name == 'stats':
        sen_dict_name = "stats_sen_dict_morris"
elif args.cal_sen == 'local' or args.method == 'local':
    cal_sen_method = '_local'
    sen_dict_name = "sen_dict_local"
else:
    cal_sen_method = ''
    if db_name == 'imdbloadbase':
...
if db_name == 'imdbloadbase':
    ...
    with open("cached_info/error_profile_dict_haibo.json", "r") as f:
...
err_files_dict[f"{get_pure_q_id(query_id, db_name)}-{args.template_id}"] = {int(k): v for k, v in err_files_dict[
    f"{get_pure_q_id(query_id, db_name)}-{args.template_id}"].items()}
...
sql_samples_path = f"./data/imdb-new/{get_pure_q_id(args.query_id, db_name)}-{args.template_id}_{args.workload}/raw_data/{get_pure_q_id(args.query_id, db_name)}-{args.template_id}_training_{args.n}.json"
with open(sql_samples_path, 'r') as f:
    data = json.load(f)
...
# if all_samples is given, the error distribution and anchors should be generated from all the samples instead of n samples
if args.all_samples:
    args.n = 1  # need to be an int, not None
    n_in_name = 'all'
else:
    n_in_name = args.n

if args.rqo_query:
    with open(args.rqo_query) as p:
        sql = p.read()
        rqo_id = args.rqo_query.split("/")[-1][:-4]
        # print(sql)
        if args.debug:
            print(f"The file is at {args.rqo_query}, filename is {rqo_id}")

if args.workload:
    workload = args.workload + '_workload/'
else:
    workload = ''
...
for i in range(num_of_base_rel + num_of_pair_rel):
    ...
    cur_err_list, cur_err_hist = prepare_error_data(db_name, query_id, sensi_dim=i, max_sel=1.0,
                                                    ...
                                                    pqo=True, template_id=args.template_id, num=n_in_name,
...
center_err = gen_center_from_err_dist(est_card, raw_card, all_basic_rels, err_info_dict, num_of_samples=1000,
                                      debug=args.debug, naive=args.naive)
# center_err = None
...
def get_plan_list(sensitive_rels):
    ...
    if args.inst_id != None:
        file = './plan/' + ON_SAMPLE + 'tmp_plan_dict_' + db_name + '_' + query_id + '_' + str(
            args.inst_id) + cal_sen_method + '.txt'
    else:
        ...
        if db_name == 'imdbloadbase':  # use the plan generated by full err profile (no train-test split)
            # if args.n:
            #     if RANK_BY_PROB:
            #         file = f'./plan/{ON_SAMPLE}pqo-entire-space/rank-by-prob/on-basetmp_plan_dict_{db_name}_q{get_pure_q_id(query_id,db_name)}-t{args.template_id}-{args.n}.txt'
            #     else:
            #         file = f'./plan/{ON_SAMPLE}pqo-entire-space/on-demand/on-basetmp_plan_dict_{db_name}_q{get_pure_q_id(query_id,db_name)}-t{args.template_id}-{args.n}.txt'
            if args.rqo_query:
                if RANK_BY_PROB:
                    file = f'./plan/{ON_SAMPLE}pqo-entire-space/rank-by-prob/{workload}on-basetmp_plan_dict_{db_name}_{rqo_id}.txt'
                else:
                    file = f'./plan/{ON_SAMPLE}pqo-entire-space/on-demand/{workload}on-basetmp_plan_dict_{db_name}_{rqo_id}.txt'
            else:
                file = f'./plan/{ON_SAMPLE}pqo-entire-space/rqo/on-basetmp_plan_dict_{db_name}_{query_id}.txt'
...
def plot_cdf_of_penalty(penalty_list, plan_list, sensitive_rels, N=1000):
    ...
    plt.xscale('log')
    plt.suptitle(f'CDF of Cost Penalty (Q{query_id.replace("q", "").replace("a", "")})', fontsize=35,
                 y=0.98)  # Adjust the y value to position the title
    plt.tight_layout()
...
def exp_penalty_by_samples(cur_plan_list, sensitive_rels, joint_error_samples, tolerance, est_base_sel=est_base_sel,
                           est_join_sel=est_join_sel, save_samples=True):
    ...
    for hint_id, hint in enumerate(cur_plan_list):
        ...
        for error in joint_error_samples:
            cost_value_with_hint, cost_value_opt, new_sel_base, new_sel_join = cal_penalty_at_sample(error=error,
                                                                                                     hint=hint,
            ...
            if save_samples and args.method == 'sobol':
                if new_sel_tuple not in sample_to_penalty_dict:
                    ...
                    probability = cal_prob_of_sample(error, sensitive_rels, est_card, raw_card, err_info_dict,
                                                     is_error_sample=True)
                    # print(new_sel_tuple)
                    probability_ = cal_prob_of_sample(samples=[new_sel_tuple],
                                                      sensitive_rels=list(err_files_dict[
                                                                              f"{get_pure_q_id(query_id, db_name)}-{args.template_id}"].keys()),
                                                      est_card=est_card,
                                                      raw_card=raw_card,
    ...
    if save_samples and args.method == 'sobol':
        ...
        if args.n:
            if RANK_BY_PROB:
                filename = f'reuse/rank-by-prob/{db_name}_q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{args.n}.json'
            else:
                filename = f'reuse/on-demand/{db_name}_q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{args.n}.json'
        if args.rqo_query:
            if RANK_BY_PROB:
                filename = f'reuse/rank-by-prob/{workload}{rqo_id}.json'
            else:
                filename = f'reuse/on-demand/{workload}{rqo_id}.json'
        else:
            filename = f'reuse/rqo/{query_id}.json'
...
def Local(relations, use_penalty=False):
    ...
    sorted_indices = [(index, round(value)) for index, value in
                      sorted(enumerate(slop_list), key=lambda x: x[1], reverse=True)]
    result = []
    for m in sorted_indices:
        result.append(
            (list(err_files_dict[f"{get_pure_q_id(query_id, db_name)}-{args.template_id}"].items())[m[0]], m[1]))
    print(slop_list)
...
def Morris(N, relations):
    ...
    recentered_opt_plan, recentered_opt_cost, recentered_base_sel, recentered_join_sel = get_opt_plan_at_sample([], [],
                                                                                                                return_sel_at_sample=True)
    ...
    for i in tqdm(range(N)):
        ...
        if i % 20 == 0:
            ...
            if USE_SALIB:
                ...
                result = []
                sorted_indices = [(index, round(value)) for index, value in
                                  sorted(enumerate(avg_penalty_of_each_dim), key=lambda x: x[1], reverse=True)]
                for m in sorted_indices:
                    result.append(
                        (list(err_files_dict[f"{get_pure_q_id(query_id, db_name)}-{args.template_id}"].items())[m[0]],
                         m[1]))
...
def sobols(N, relations):
    ...
    default_opt_plan, default_opt_cost, _, _ = get_opt_plan_at_sample([], [], center_err=None,
                                                                      return_sel_at_sample=True)
    ...
    if False:
        ...
        print(np.mean(Y_default), np.mean(Y_recenter), np.mean(Y_rob1), np.mean(Y_rob2), np.mean(Y_rob3),
              np.mean(Y_related_work))
        # quit()
    ...
    while len(Y) > 100:
        ...
        result = []
        sorted_s1 = [(index, value) for index, value in
                     sorted(enumerate(Si['S1']), key=lambda x: abs(x[1]), reverse=True)]

        for i in sorted_s1:
            result.append(
                (list(err_files_dict[f"{get_pure_q_id(query_id, db_name)}-{args.template_id}"].items())[i[0]], i[1]))
        logging.info(result)
        print(sorted_s1)
        print(result)
        ...
        if len(sorted_s1) < 5:
            print(len(sorted_s1))
            top_n_sensitive_dim = top_n_of_2d_matrix(Si['S2'], len(sorted_s1))  # print the id as (id_pair_rel + base)
        else:
        ...
        if iter == 0:
            with open("cached_info/sensitive_dict.json", 'r') as f:
                data = json.load(f)
            data[sen_dict_name][args.workload + "-od-" + rqo_id] = [dim[0][0] for dim in result[:3]]
            with open("cached_info/sensitive_dict.json", 'w') as f:
...
def cal_dim_sensitivity(start_time=None):
    global sensitive_rels

    # dimension_space = list(range(num_of_base_rel, num_of_base_rel + num_of_pair_rel))
    dimension_space = list(err_files_dict[f"{get_pure_q_id(query_id, db_name)}-{args.template_id}"].keys())
    ...
    if args.cal_sen == 'local':
        ...
        if query_id in ["28a", "30a", "31a", "33a"]:  # dimension number is large, reduce sample size
            sobols(N=64, relations=dimension_space)
        else:
            sobols(N=128, relations=dimension_space)
    if start_time:
        logging.info(f"Calculating sensitive dimension for {rqo_id}: {time.time() - start_time}")
    quit()
...
def rqo():
    # if args.n:
    #     if RANK_BY_PROB:
    #         logging.basicConfig(filename='./log/' + ON_SAMPLE + 'rank-by-prob/anchors/' + f'{db_name}_q{get_pure_q_id(query_id,db_name)}-t{args.template_id}-{args.n}' + '_' + str(tolerance) + cal_sen_method + '.log', level=logging.INFO)
    #     else:
    #         logging.basicConfig(filename='./log/' + ON_SAMPLE + 'on-demand/anchors/' + f'{db_name}_q{get_pure_q_id(query_id,db_name)}-t{args.template_id}-{args.n}' + '_' + str(tolerance) + cal_sen_method + '.log', level=logging.INFO)
    if args.rqo_query:
        if RANK_BY_PROB:
            logging.basicConfig(
                filename='./log/' + ON_SAMPLE + 'rank-by-prob/anchors/' + workload + db_name + '_' + rqo_id + '_' + str(
                    tolerance) + cal_sen_method + '.log', level=logging.INFO)
        else:
            logging.basicConfig(
                filename='./log/' + ON_SAMPLE + 'on-demand/anchors/' + workload + db_name + '_' + rqo_id + '_' + str(
                    tolerance) + cal_sen_method + '.log', level=logging.INFO)
    else:
        logging.basicConfig(filename='./log/' + ON_SAMPLE + 'rqo/anchors/' + db_name + '_' + query_id + '_' + str(
            tolerance) + cal_sen_method + '.log', level=logging.INFO)
    ...
    if not args.cal_sen:
        ...
        if args.rqo_query:
            if RANK_BY_PROB:
                sensitive_rels = sorted(sen_dict[args.workload + "-rbp-" + rqo_id])  # read from file
            else:
                sensitive_rels = sorted(sen_dict[args.workload + "-od-" + rqo_id])  # read from file
        else:
        ...
        sensitive_rels = cal_dim_sensitivity(start_time=start_time)
    ...
    while 0:
        ...
        ori_opt_plan, ori_opt_cost, base, join = get_opt_plan_at_sample([0.02, 1.50], [13, 5], center_err=None,
                                                                        return_sel_at_sample=True)
        ...
        # print(ori_opt_cost)        
        ori_latency = get_real_latency(db_name, sql, hint=ori_opt_plan, times=10, inject=False, output_plan=False,
                                       limit_time=10000)
        ...
        new_latency = get_real_latency(db_name, sql, hint=new_opt_plan, times=10, inject=False, output_plan=False,
                                       limit_time=10000)
        print(new_opt_cost, new_latency)
        input()

        recentered_opt_plan, recentered_opt_cost, _, _ = get_opt_plan_at_sample(center_err, all_basic_rels,
                                                                                return_sel_at_sample=True)
        plan = recentered_opt_plan  # set the hint to current best plan id
        print("### Recenter by err: ", center_err, recentered_opt_cost)
        print("### Cost after recentered (inject the plan): ",
              get_plan_cost(cursor, sql, plan, explain="EXPLAIN (COSTS, FORMAT JSON)"))
        print("### Opt cost at recentered (optimizer's default): ",
              get_plan_cost(cursor, sql, explain="EXPLAIN (COSTS, FORMAT JSON)"))
        input()
        ...
        print("### Cost of new opt plan: ",
              get_plan_cost(cursor, sql, new_opt_plan, explain="EXPLAIN (COSTS, FORMAT JSON)"))
    ...
    logging.info(
        f"Instance id = {args.inst_id}, Error div = {args.div}, bandwidth = {args.b}, tolerance = {tolerance}, have {args.sample} samples.")
    ...
    if args.exe:
        ...
        default_plan_latency = round(get_real_latency(db_name, sql, times=exe_times, limit_time=1200000), 1)
        logging.info(f'postgres default latency: {default_plan_latency}')
        recentered_opt_plan, recentered_opt_cost, recentered_base_sel, recentered_join_sel = get_opt_plan_at_sample([],
                                                                                                                    [],
                                                                                                                    center_err=center_err,
                                                                                                                    return_sel_at_sample=True)
        recentered_opt_plan_latency = round(
            get_real_latency(db_name, sql, hint=recentered_opt_plan, times=exe_times, limit_time=1200000), 1)
        ...
        for i in range(len(cur_plan_list)):
            # for i in [6]:
            latency = round(get_real_latency(db_name, sql, hint=cur_plan_list[i], times=exe_times, limit_time=1200000),
                            1)
            cur_plan_latency_list.append(latency)
            print(i, ": ", latency)
            logging.info(f"{i}: {latency}")
        logging.info(
            f"Best latency we know is: {min(cur_plan_latency_list)} ms, with plan id = {cur_plan_latency_list.index(min(cur_plan_latency_list))}, {cur_plan_latency_list} ")
        # quit()
    ...
    for i, sensitive_rels in enumerate(sensitive_rels_trys):
        ...
        joint_err_samples = gen_samples_from_joint_err_dist(args.sample, relations=sensitive_rels, random_seeds=True,
                                                            naive=args.naive)

        logging.info(f"The number of samples to calculate expected penalty = {args.sample}.")
        exp_penalty_list, std_penalty_list, prob_of_penal = exp_penalty_by_samples(cur_plan_list, sensitive_rels,
                                                                                   joint_err_samples,
                                                                                   tolerance=tolerance,
                                                                                   save_samples=True)
        ...
        logging.info(f"### Best plan detial by exp penalty:")
        # logging.info(f"{cur_plan_list[robust_plan_id]}")
        logging.info(f"exp_penalty_w_tol: \t{[(id_, i_) for id_, i_ in enumerate(exp_penalty_list)]}")

        # caching the robust plan
        with open("cached_info/robust_plan_dict.json", 'r') as f:
            data = json.load(f)
        if RANK_BY_PROB:
            key = args.workload + '-rbp-' + rqo_id
        else:
            key = args.workload + '-od-' + rqo_id
        data["cached_rob_plan_dict"][key] = [robust_plan_id[0]]
        ...
        conn.close()
        logging.info(f"RQO for {rqo_id}: {time.time() - start_time}")


### Future Work: use a subset of query to generate several robust plans and cover the entire space
def pqo_query_subset(N=100):
    logging.basicConfig(
        filename='./log/' + ON_SAMPLE + "overhead/" + f"Finding-achors-{query_id}-{args.template_id}-{n_in_name}-{args.workload}" + '.log',
        level=logging.INFO)
    # sql_template, literal_list = gen_sql_by_template(query_id, N)
    sample_freq_path, sample_path = None, None
    if args.n:
        sample_freq_path = f"./data/imdb-new/{get_pure_q_id(query_id, db_name)}-{args.template_id}_{args.workload}/sample-{n_in_name}.csv"
        sample_path = f"./data/imdb-new/{get_pure_q_id(query_id, db_name)}-{args.template_id}_{args.workload}/raw_data/{get_pure_q_id(query_id, db_name)}-{args.template_id}_{n_in_name}_training.txt"
    # sql_template, literal_list = gen_sql_by_template(get_pure_q_id(query_id, db_name), N, db_name, rank_by_prob=RANK_BY_PROB,
    #                                                  sample_freq_path=sample_freq_path,
    #                                                  sample_path=sample_path, t=args.template_id)

    # Load query from training data
    sample_query_path = f"./data/imdb-new/{get_pure_q_id(query_id, db_name)}-{args.template_id}_{args.workload}/raw_data/{get_pure_q_id(query_id, db_name)}-{args.template_id}_training_{n_in_name}.json"
    with open(sample_query_path, "r") as file:
        data = json.load(file)
    sql_template = [data[key] for key in data.keys()]

    if not args.all_samples:
        sql_template = sql_template[:args.n]
    else:
    ...
    if args.n:
        if RANK_BY_PROB:
            filename = f'reuse/rank-by-prob/anchors/{workload}q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}-optimized-queries-b{args.b}.json'
        else:
            filename = f'reuse/on-demand/anchors/{workload}q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}-optimized-queries-b{args.b}.json'
    else:
        if RANK_BY_PROB:
            filename = f'reuse/rank-by-prob/anchors/{get_pure_q_id(query_id, db_name)}-optimized-queries-b{args.b}.json'
        else:
            filename = f'reuse/on-demand/anchors/{get_pure_q_id(query_id, db_name)}-optimized-queries-b{args.b}.json'
    print(filename)
    # if os.path.exists(filename):
    #     return    
    for sql_id, para_sql in tqdm(enumerate(sql_template)):
        ### Process new sql
        para_table_name_id_dict, para_join_maps, para_join_info, para_pair_rel_info = get_maps(db_name, para_sql,
                                                                                               debug=args.debug)
        ...
        for q_id, optimized_query in enumerate(optimized_queries_dict.values()):
            ...
            kl_val = cal_kl(err_info_dict, optimized_query[0], optimized_query[1],
                            ...
                            dim=list(err_files_dict[f"{get_pure_q_id(query_id, db_name)}-{args.template_id}"].keys()),
                            N=1)

            if math.exp(kl_val) < 500:
                if kl_val < min_kl:
                    min_kl = kl_val
                    nearest_id = q_id

        if nearest_id == -1:
            optimized_queries_dict[len(optimized_queries_dict)] = [[int(x) for x in para_est_card],
                                                                   [int(x) for x in para_raw_card], para_sql]
    with open(filename, 'w') as file:
    ...
    for q_id, optimized_query in enumerate(optimized_queries_dict.values()):
        if args.n:
            if RANK_BY_PROB:
                filename = f'query/join-order-benchmark/rank-by-prob/{workload}q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}-{q_id}-b{args.b}.sql'
            else:
                filename = f'query/join-order-benchmark/on-demand/{workload}q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}-{q_id}-b{args.b}.sql'
        else:
            if RANK_BY_PROB:
                filename = f'query/join-order-benchmark/rank-by-prob/{get_pure_q_id(query_id, db_name)}-{q_id}-b{args.b}.sql'
            else:
                filename = f'query/join-order-benchmark/on-demand/{get_pure_q_id(query_id, db_name)}-{q_id}-b{args.b}.sql'
        with open(filename, 'w') as sql_file:
            sql_file.write(optimized_query[2])
    logging.info(
        f"Finding anchors for {query_id}-{args.template_id}-{n_in_name}-{workload}: {time.time() - start_time}")
    return


# Robust parametrized query optimization
def pqo(N=100):
    conn = psycopg2.connect(host="/tmp", dbname=db_name, user="hx68")
    conn.set_session(autocommit=True)
    cursor = conn.cursor()
    if args.n:
        if RANK_BY_PROB:
            filename = './log/' + ON_SAMPLE + 'rank-by-prob/pqo/' + workload + f'{db_name}_q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}' + '_pqo_' + str(
                tolerance) + cal_sen_method + '.log'
        else:
            filename = './log/' + ON_SAMPLE + 'on-demand/pqo/' + workload + f'{db_name}_q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}' + '_pqo_' + str(
                tolerance) + cal_sen_method + '.log'
    else:
        if RANK_BY_PROB:
            filename = './log/' + ON_SAMPLE + 'rank-by-prob/pqo/' + db_name + '_' + query_id + '_pqo_' + str(
                tolerance) + cal_sen_method + '.log'
        else:
            filename = './log/' + ON_SAMPLE + 'on-demand/pqo/' + db_name + '_' + query_id + '_pqo_' + str(
                tolerance) + cal_sen_method + '.log'
    ...
    if args.n:
        sql_template_name = f"./data/imdb-new/{get_pure_q_id(query_id, db_name)}-{args.template_id}_{args.workload}/raw_data/{get_pure_q_id(query_id, db_name)}-{args.template_id}_testing.json"
        with open(sql_template_name, 'r') as file:
            sql_template = json.load(file)
            sql_template = list(sql_template.values())
    ...
    for sql_id, para_sql in enumerate(sql_template):
        ...
        if os.path.exists(filename):
            ...
            for i in range(len(optimized_queries_dict.values())):
                if args.n:
                    if RANK_BY_PROB:
                        cached_samples_filename = f'reuse/rank-by-prob/{workload}q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}-{i}-b{args.b}.json'
                    else:
                        cached_samples_filename = f'reuse/on-demand/{workload}q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}-{i}-b{args.b}.json'
                else:
                    if RANK_BY_PROB:
                        cached_samples_filename = f'reuse/rank-by-prob/{get_pure_q_id(query_id, db_name)}-{i}-b{args.b}.json'
                    else:
                        cached_samples_filename = f'reuse/on-demand/{get_pure_q_id(query_id, db_name)}-{i}-b{args.b}.json'
                ...
                if args.n:
                    if RANK_BY_PROB:
                        plan_file = f'./plan/{ON_SAMPLE}pqo-entire-space/rank-by-prob/{workload}on-basetmp_plan_dict_{db_name}_q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}-{i}{cal_sen_method}-b{args.b}.txt'
                    else:
                        plan_file = f'./plan/{ON_SAMPLE}pqo-entire-space/on-demand/{workload}on-basetmp_plan_dict_{db_name}_q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}-{i}{cal_sen_method}-b{args.b}.txt'
                else:
                    if RANK_BY_PROB:
                        plan_file = f'./plan/{ON_SAMPLE}pqo-entire-space/rank-by-prob/on-basetmp_plan_dict_{db_name}_{get_pure_q_id(query_id, db_name)}-{i}{cal_sen_method}-b{args.b}.txt'
                    else:
                        plan_file = f'./plan/{ON_SAMPLE}pqo-entire-space/on-demand/on-basetmp_plan_dict_{db_name}_{get_pure_q_id(query_id, db_name)}-{i}{cal_sen_method}-b{args.b}.txt'
            ...
            kl_val_list = [cal_kl(err_info_dict, cached_query[0], cached_query[1],
                                  err_info_dict, para_est_card, para_raw_card,
                                  dim=list(
                                      err_files_dict[f"{get_pure_q_id(query_id, db_name)}-{args.template_id}"].keys()),
                                  N=1, debug=False)
            ...
            ### Haibo TODO: change nearest to top K nearest neighbors

            cached_samples_dict = cached_samples_dict_list[nearest_query_id]
            # if args.n:
            #     key = f"{args.workload}-q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}-{nearest_query_id}-b{args.b}"
            #     print(key)
            #     if RANK_BY_PROB:
            #         plan_space = cached_rob_plan_dict_by_prob[key]
            #     else:
            #         plan_space = cached_rob_plan_dict_on_demand[key]
            # else:
            #     key = f"{get_pure_q_id(query_id, db_name)}-{nearest_query_id}-b{args.b}"
            #     print(key)
            #     if RANK_BY_PROB:
            #         plan_space = cached_rob_plan_dict_by_prob[key]
            #     else:
            ...
            with open("cached_info/robust_plan_dict.json", 'r') as f:
                data = json.load(f)
            if RANK_BY_PROB:
                key = f"{args.workload}-rbp-q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}-{nearest_query_id}-b{args.b}"
            else:
                key = f"{args.workload}-od-q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}-{nearest_query_id}-b{args.b}"
            plan_space = data["cached_rob_plan_dict"][key]
            ...
            logging.info(
                f"--- PQO: Need {math.exp(kl_val)} samples to be accurate, We have {len(cached_samples_dict.keys())}")
            ...
            # Haibo TODO: Instead of using the robust plan from the nearest, 
            # use the most common robust plan among the K neighbors
            ...
            if len(plan_space) > 1:
                ...
                for id, reuse_sample in enumerate(cached_samples_dict.keys()):
                    ...
                    for plan_id in plan_space:
                        est_penalty = (p_new / p_old) * cached_samples_dict[reuse_sample][
                            plan_id + 1]  ## plan_id+1 because the 1st in the list is probability
                ...
                rob_plan_id = \
                    [plan_space[i] for i, x in enumerate(est_exp_penalty_list) if x == min(est_exp_penalty_list)][0]
            else:
                rob_plan_id = plan_space[0]
            print(f"--- PQO: {sql_id}: most robust plan is {rob_plan_id}, nearest query is {nearest_query_id}")
            result.append([sql_id, para_sql, cur_plan_list[rob_plan_id]])
            info = {"sql_id": sql_id,
                    "nearest_query": nearest_query_id,
                    "robust_plan": rob_plan_id,
                    "kl_val": kl_val,
                    'sql': para_sql,
                    'hint': cur_plan_list
                    }
        ...
        #     pg_latency = round(get_real_latency(db_name, para_sql, hint=None, times=5, limit_time=1000000), 5)
        #     rob_latency = round(get_real_latency(db_name, para_sql, hint=cur_plan_list[rob_plan_id], times=5, limit_time=100000), 5)
        ...
        # if args.n:
        #     if RANK_BY_PROB:
        #         filename = f'reuse/{db_name}/rank-by-prob/{workload}pqo-q{get_pure_q_id(query_id,db_name)}-t{args.template_id}-{n_in_name}-b{args.b}.txt'
        #     else:
        #         filename = f'reuse/{db_name}/on-demand/{workload}pqo-q{get_pure_q_id(query_id,db_name)}-t{args.template_id}-{n_in_name}-b{args.b}.txt'
        # else:
        #     if RANK_BY_PROB:
        #         filename = f'reuse/{db_name}/rank-by-prob/pqo-{query_id}-b{args.b}.txt'
        #     else:
        #         filename = f'reuse/{db_name}/on-demand/pqo-{query_id}-b{args.b}.txt'
        # write_to_file(result, filename)
        logging.info(f"PQO find all query plans for {query_id}_{n_in_name}_{workload}: {time.time() - start_time}")

        with open(
                f'reuse/{db_name}/on-demand/{workload}pqo-q{get_pure_q_id(query_id, db_name)}-t{args.template_id}-{n_in_name}-b{args.b}.csv',
        ...
        # with open(f'reuse/{db_name}/on-demand/{workload}pqo-q{get_pure_q_id(query_id,db_name)}-t{args.template_id}-{n_in_name}-b{args.b}.json', 'w') as json_file:
...
def basic_pqo(N=100):
    ...
    conn.set_session(autocommit=True)
    cursor = conn.cursor()
    logging.basicConfig(filename='./log/' + ON_SAMPLE + db_name + '_' + query_id + '_pqo_' + str(tolerance) + '.log',
                        level=logging.INFO)
    ...
    for sql_id, para_sql in enumerate(sql_template):
        ...
        for ii in plan_space:
            print(
                f"Our plan's cost: {get_plan_cost_simple(cursor, hint=cur_plan_list[ii], sql=para_sql, explain=explain, debug=True)}")
        ...
        kl_val = cal_kl(err_info_dict, est_card, raw_card,
                        err_info_dict, para_est_card, para_raw_card,
                        dim=list(err_files_dict[f"{get_pure_q_id(query_id, db_name)}-{args.template_id}"].keys()), N=1)
        ...
        if len(plan_space) > 1:
            ...
            for id, reuse_sample in enumerate(converted_dict.keys()):
                ...
                for plan_id in plan_space:
                    est_penalty = (p_new / p_old) * converted_dict[reuse_sample][
                        plan_id + 1]  ## plan_id+1 because the 1st in the list is probability
                    if plan_id not in est_penalty_dict:
                        est_penalty_dict[plan_id] = [est_penalty]
                    else:
            ...
            # continue
            logging.info(est_exp_penalty_list)
            rob_plan_id = [plan_space[i] for i, x in enumerate(est_exp_penalty_list) if x == min(est_exp_penalty_list)][
                0]
        else:
            rob_plan_id = plan_space[0]
        ...
        rob_latency = round(
            get_real_latency(db_name, para_sql, hint=cur_plan_list[rob_plan_id], times=5, limit_time=100000), 1)
        # test_latency = round(get_real_latency(db_name, para_sql, hint=cur_plan_list[plan_space[0]], times=3, limit_time=10000), 1)
        # test_latency_ = round(get_real_latency(db_name, para_sql, hint=cur_plan_list[plan_space[1]], times=3, limit_time=10000), 1)
        # test_latency__ = round(get_real_latency(db_name, para_sql, hint=cur_plan_list[plan_space[2]], times=3, limit_time=10000), 1)
        # logging.info(f"--- test latency is {test_latency}, {test_latency_}, {test_latency__}")
