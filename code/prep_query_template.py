...
def gen_sql_by_template(query_id, K, db_name=None, rank_by_prob=False, sample_path=None, sample_freq_path=None, t=None):
    ...
    # Modified 7a to be 7, so 7a is missing
    ...
    if query_id == '1':
        for i in sampled_literals:
            i[2] = np.random.choice(available_it_for_miidx)
        sql_list = [change_query_1_0(i) for i in sampled_literals]
    ...
    if query_id == '6':
        sql_list = [change_query_6_0(i) for i in sampled_literals]

    if query_id == "7":
        for i in sampled_literals:
            i[1] = np.random.choice(available_it_for_pi)
        sql_list = [change_query_7_0(i) for i in sampled_literals]

    if query_id == '8':
        sql_list = [change_query_8_0(i) for i in sampled_literals]
    if query_id == '9':
        sql_list = [change_query_9_0(i) for i in sampled_literals]
    if query_id == '10':
        sql_list = [change_query_10_0(i) for i in sampled_literals]
    if query_id == '11':
    ...
    if query_id == '18':
        for i in sampled_literals:
            i[1] = np.random.choice(available_it_for_mi)
            i[2] = np.random.choice(available_it_for_miidx)
        sql_list = [change_query_18_0(i) for i in sampled_literals]

    if query_id == '20':
        sql_list = [change_query_20_0(i) for i in sampled_literals]
    if query_id == '21':
        sql_list = [change_query_21_0(i) for i in sampled_literals]
...
def change_query_20_0(literals):
    sql = f'''
        ...
        FROM complete_cast AS cc,
            ...
            name AS n,
            title AS t
        WHERE {literals[0].replace("cct", "cct1")}
        AND {literals[1].replace("cct", "cct2")}
        AND {literals[2]}
        AND {literals[3]}
        AND {literals[4]}
        AND {literals[5]}
